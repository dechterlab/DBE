...
int32_t ARE::ARP::LoadFromFile(const std::string & FileName)
{
	...
	if ("gr" == fn_ext) {
		...
		Destroy();
        printf("Line num 443, after destroy");
	    ...
	    printf("File -- %s",p1);
		FILE *fp = fopen(p1, "rb") ;
        printf("Line num 512 - file pointer to file %s", FileName.c_str());
        ...
        printf("Line num 519 , opened file");
...
//#define bufsize	33554432
        ...
        printf("Line num 535, buffer allocated");
        ...
        printf("Line num 549, res value :%d", res);
